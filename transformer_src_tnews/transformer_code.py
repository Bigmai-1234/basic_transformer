...
class Transformer(object):
    ...
    def __init__(self, config):
        ...
        # 定义多头l3损失
        self.l3Loss = tf.constant(0.0)

        # 词嵌入层, 位置向量的定义方式有两种：一是直接用固定的one-hot的形式传入，然后和词向量拼接，在当前的数据集上表现效果更好。另一种
        # 就是按照论文中的方法实现，这样的效果反而更差，可能是增大了模型的复杂度，在小数据集上表现不佳。

        with tf.name_scope("embedding"):
            wordEmbedding = tf.Variable(tf.random_uniform([config.vocab_len, config.model.embeddingSize]))
            # 利用预训练的词向量初始化词嵌入矩阵
            self.W = tf.Variable(tf.cast(wordEmbedding, dtype=tf.float32, name="random_embedding"), name="W")
            # 利用词嵌入矩阵将输入的数据中的词转换成词向量，维度[batch_size, sequence_length, embedding_size]
            self.embedded = tf.nn.embedding_lookup(self.W, self.inputX)
            # print(self.embedded.shape)
            # print(self.embeddedPosition.shape)
        ...
        with tf.name_scope("transformer"):
            for i in range(config.model.numBlocks):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    ...
                    multiHeadAtt, tmpLoss = self._multiheadAttention(rawKeys=self.inputX, queries=self.embeddedWords,
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = tf.reduce_mean(losses) + config.model.l2RegLambda * l2Loss + self.l3Loss
    ...
    def _multiheadAttention(self, rawKeys, queries, keys, numUnits=None, causality=False, scope="multiheadAttention"):
        ...
        #####
        # [batch_size, sequence_length, embedding_size/numHeads]
        Q_split = tf.split(Q_, numHeads, axis=0)
        # 每一个头既保持类内距离（空间内特征维度正交），
        # 也保持类间距离（不同空间正交）
        K_split = tf.split(K_, numHeads, axis=0)
        V_split = tf.split(V_, numHeads, axis=0)

        tmp_loss = self._compute_loss(Q_split, numHeads) + self._compute_loss(K_split, numHeads) + \
              self._compute_loss(V_split, numHeads)
    ...
    # def _compute_loss(self, feature_matrix, numHeads):
    #     _tmp = feature_matrix[0]
    #     for i in range(1, numHeads):
    #         if i % 2 != 0:
    #             _tmp = tf.matmul(_tmp, tf.transpose(feature_matrix[i], [0, 2, 1]))
    #         else:
    #             _tmp = tf.matmul(_tmp, feature_matrix[i])
    #
    #     # 线性变换至两维，彼此差异最大
    #     _tmp = tf.layers.dense(_tmp, 2, activation=tf.nn.relu)
    #     loss = kld(_tmp[:,:,0], _tmp[:,:,1]) + kld(_tmp[:,:,1], _tmp[:,:,0])
    #     return loss * 0.5

    def _compute_loss(self, feature_matrix, numHeads):
        sum_matrix_norm = tf.norm(tf.math.add_n(feature_matrix))
        numHeads_norm = tf.math.add_n([tf.norm(i) for i in feature_matrix])
        return sum_matrix_norm/numHeads_norm
