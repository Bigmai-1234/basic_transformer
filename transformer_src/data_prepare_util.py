...
from utils import process_context
# 数据预处理的类，生成训练集和测试集
...
class Dataset(object):
    ...
    def _readData(self, filePath):
        ...
        if self.config.numClasses == 1:
            labels = df["first_c"].tolist()
        elif self.config.numClasses > 1:
            labels = df["second_c"].tolist()

        review = df["context"].apply(process_context).tolist()
    ...
    def _getWordEmbedding(self, words):
        ...
        wordVec = gensim.models.KeyedVectors.load_word2vec_format("../word2vec/xf_word2Vec.bin", binary=True)
        vocab = []
        wordEmbedding = []
        ...
        for word in words:
            try:
                vector = wordVec.get_vector(word)
                vocab.append(word)
